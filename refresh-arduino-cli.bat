...
if defined ARDUINO_IDE_LIBRARIES (
    "%ARDUINO_CLI%" --config-file "%ARDUINO_CONFIG_FILE%" compile --library "%WORKSPACE_LIBRARY_ROOT%" --libraries "%ARDUINO_IDE_LIBRARIES%" --verbose "%~1"
) else (
    "%ARDUINO_CLI%" --config-file "%ARDUINO_CONFIG_FILE%" compile --library "%WORKSPACE_LIBRARY_ROOT%" --verbose "%~1"
)
echo Uploading %1
